...
<div class="bg-primary h-fit p-4 sm:p-8 sm:px-16 w-screen" >
	...
	<div class="z-30 flex justify-between">
		...
		{#if showMenu}
			<button onclick={toggleMenu}  class:active={showMenu}>
				<img src="closeMenu.svg" class="h-6 w-6 sm:h-10 sm:w-10" alt="Close Website Directory" />
			</button>
		{:else}
			<button onclick={toggleMenu}>
				<img src="menuBars.svg" class="h-6 w-6 sm:h-10 sm:w-10" alt="Open Website Directory" />
			</button>
		{/if}
	</div>
	...
	{#if showMenu}
		<div
			...
			class="bg-tertiary text-darkText fixed top-14 sm:top-18 right-0 flex h-screen w-full sm:w-1/2 md:w-1/4 flex-col p-12"
		>
