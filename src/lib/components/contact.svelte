<script lang="ts">
	...
	// Initialize dropdown only in browser
	onMount(() => {
	...
	function setActivePosition(tab: 'server' | 'foodRunner' | 'driver' | 'kitchenHelper') {
		activePosition = tab;
		// Close dropdown after selection
		if (dropdown) {
			dropdown.hide();
		...
		<div class="mx-8 my-12 flex w-full flex-col items-start xl:my-24 xl:ml-16">
			<div class=" mr-16 mb-4 flex flex-col xl:mr-48">
				...
				<button
					...
					class="bg-tertiary font-MonaSans border-darkText text-darkText mt-8 inline-flex items-center rounded-[5px] border p-1 text-[1rem]"
					type="button"
				>
					{activeTab === 'general' ? 'General' : 'Apply'} - Select option:
					<svg
				...
				<div
					id="dropdownMenu"
					class="bg-tertiary text-darkText border-darkText font-MonaSans z-10 mx-16 hidden w-full divide-y divide-gray-100 rounded-[5px] border"
				>
					<ul class="text-darkText py-2 text-sm" aria-labelledby="dropdownButton">
						<li>
							<button
								class="block w-full px-4 py-2 text-left hover:bg-gray-100 {activeTab === 'general'
						...
						<li>
							<button
								class="block w-full px-4 py-2 text-left hover:bg-gray-100 {activeTab ===
								'application'
									? 'bg-gray-200 font-semibold'
									: ''}"
					...
					>
						...
						<div class="mt-8 grid grid-cols-2 grid-rows-2">
							<input
								...
								class="border-darkText bg-tertiary mt-1 mr-1 mb-1 rounded-[5px] border-1 p-1"
							/>
							<input
								type="text"
								name="Last Name"
								...
								class="border-darkText bg-tertiary mt-1 mb-1 ml-1 rounded-[5px] border-1 p-1"
							/>
							<input
								type="text"
								name="Email Address"
				...
				{:else if activeTab === 'application'}
					...
					>
						<input type="hidden" name="access_key" value="fde62125-4609-4617-98ce-76bf3ef7159e" />
						<div class="mt-8 grid grid-cols-2 grid-rows-3">
							<input
								type="text"
								name="First Name"
								placeholder="First Name"
							...
							<div>
								<button
									...
									data-dropdown-toggle="dropdown"
									class="border-darkText bg-tertiary mt-1 mr-1 mb-1 w-full rounded-[5px] border-1 p-1"
									type="button"
								>
									{activePosition === 'server' ? 'Server' : 'Food'} - Select option:
									<svg
										class="ms-3 h-2.5 w-2.5"
										aria-hidden="true"
										xmlns="http://www.w3.org/2000/svg"
								...
								>
									<ul class="text-darkText py-2 text-sm" aria-labelledby="dropdownButton">
										<li>
											<input
												class="block w-full px-4 py-2 text-left hover:bg-gray-100 {activePosition ===
												'server'
													? 'text-darkText'
													: 'text-lightText'}"
							...
							<input
								...
								class="border-darkText bg-tertiary file:bg-tertiary file:text-darkText my-1 w-69 rounded-[5px] border-1 p-1 pl-5 file:rounded-[5px] file:border file:p-1 file:text-[1rem] file:font-normal"
							/>
						</div>
						<div class="">
							<textarea
